            ...
            <div className="flex items-center gap-3 ">
                <ul className={`md:flex items-center gap-3 md:visible ${isMenuHamOpen ? 'block' : 'hidden'}`}>
                    <li className={`font-semibold ${hasScrolled ? 'text-black' : 'text-white'} `}>
                        {location.pathname === '/' ? (
                            <a href="#quienes-somos"
                                className="hover:underline hover:underline-offset-4"> {/*className={({ isActive }) => isActive ? actvieStyle : undefined}>*/}
                                Quiénes somos
                            </a>
                        ) : (
                            <NavLink
                    ...
                    <li>
                        ...
                        gap-x-1.5 rounded-md bg-transparent  font-medium ring-1 ring-inset ring-transparent hover:underline
                        hover:underline-offset-4 ${hasScrolled ? 'text-black' : 'text-white'}`}
                                id="menu-button"
                                aria-expanded={isMenuOpen}
                                aria-haspopup="true"
                                onMouseEnter={() => {
                                    if (svgRef.current) {
                                        svgRef.current.classList.add('translate-y-1');
                                    }
                                }}
                                onMouseLeave={() => {
                                    if (svgRef.current) {
                                        svgRef.current.classList.remove('translate-y-1');
                                    }
                                }}
                            >
                                Recursos
                                <svg ref={svgRef}
                                    className={`-mr-1 h-5 w-5  ${hasScrolled ? 'text-black' : 'text-white'}
